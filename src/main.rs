...
#[post("/new_order")]
async fn new_order(pool: web::Data<DbPool>, data: web::Json<Vec<models::NewOrder>>) -> HttpResponse {
    let conn = pool.get()
        .expect("Impossible to connect to db");
    ...
    let result = 
        web::block(move || mylib::insert_order(&conn, data.into_inner()))
            .await
            .map_err(|e| {
                eprintln!("{}", e);
                HttpResponse::InternalServerError().finish()
    ...
    match result {
        Ok(value) => HttpResponse::Created().body(format!("{}", value)),
        Err(_) => HttpResponse::InternalServerError().finish()
    }
}
